...
class DrumPad extends React.Component {
  ...
  render() {
    const keyButtons = this.props.keys.map(key => 
                                           (<button onClick={this.handleClick} class="drum-pad" id={key.descriptor}>
      <audio  class="clip" id={key.label} src={key.audio}>{key.descriptor}</audio>
      {key.label}
     </button>));
...
class DrumMachine extends React.Component {
 ...
 handleInput(text) {
  ...
  render() {
    return (
      <div id="drum-machine">
        <DrumPad keys={this.props.keys} onInput={this.handleInput}/>
...
ReactDOM.render(<DrumMachine keys={keys}/>, document.querySelector(".container-fluid"));